...
const getEnvOrLocalSupabase = () => {
  const localUrl = typeof window !== 'undefined' ? localStorage.getItem('fitflow_supabase_url') : null;
  const localKey = typeof window !== 'undefined' ? localStorage.getItem('fitflow_supabase_key') : null;
...
const activeSupabase = getEnvOrLocalSupabase();

export const isSupabaseConfigured = activeSupabase.isConfigured;

export const supabase: SupabaseClient | null = isSupabaseConfigured
  ? createClient(activeSupabase.url, activeSupabase.key)
  : null;

export function saveCustomSupabaseConfig(url: string, key: string) {
  if (url.trim() && key.trim()) {
    localStorage.setItem('fitflow_supabase_url', url.trim());
    localStorage.setItem('fitflow_supabase_key', key.trim());
  } else {
    localStorage.removeItem('fitflow_supabase_url');
    localStorage.removeItem('fitflow_supabase_key');
  }
...
export function getCurrentSupabaseConfig() {
  const localUrl = typeof window !== 'undefined' ? localStorage.getItem('fitflow_supabase_url') : '';
  const localKey = typeof window !== 'undefined' ? localStorage.getItem('fitflow_supabase_key') : '';
